...
class AI ():
    ...
    def update_Qtable(self, state, action, pos1, turn, pos2, statep1):
        ...
        #UP
        if action == 0:
            qtable.up_score = qtable.up_score + 0.75 * (reward + 0.9 * score_p1 - qtable.up_score)
        #LEFT
        elif action == 1:
            qtable.left_score = qtable.left_score + 0.75 * (reward + 0.9 * score_p1 - qtable.left_score)
        #DOWN
        elif action == 2:
            qtable.down_score = qtable.down_score + 0.75 * (reward + 0.9 * score_p1 - qtable.down_score)
        #RIGHT
        elif action == 3:
            qtable.right_score = qtable.right_score + 0.75 * (reward + 0.9 * score_p1 - qtable.right_score)
