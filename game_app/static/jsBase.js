...
let token2
let color2
let player1color = "playerDefault1"
let player2color = "playerDefault2"


function setSettings(){

        if(token1 != undefined)
                player_1.icon = settings['tokens'][token1]

        if(token2 != undefined)
                player_2.icon = settings['tokens'][token2]

        if(color1 != undefined)
                player1color = color1 + "Player"
        
        if(color2 != undefined && color2 != color1)
                player2color = color2 + "Player"
...
function startGame(url){
        ...
        token1 = formData.getAll('token1')[0]
        color1 = formData.getAll('color1')[0]
        token2 = formData.getAll('token2')[0]
        color2 = formData.getAll('color2')[0]
        sets = document.getElementById("settings")
        document.getElementsByClassName('mainDiv')[0].removeChild(sets)
...
function displayPossibleMove(){
        ...
        opponentPlayer = turn == 1 ? player2color : player1color
                ...
                for(let y in state_board[i]){
                        ...
                        switch(state_board[i][y]){
                                case 0 :
                                        ...
                                        break;
                                case 1 :
                                        table.children[i].children[y].className = player1color
                                        break;
                                case 2 :
                                        table.children[i].children[y].className = player2color
                                        break;
                        }
